...
- **Stages 1-7, 13 DONE.** Membership PDP, Build a Bundle, Single Issue PDPs, /shop, Cart, Checkout, Filemonk digital delivery all live. Mags (Mag01-Mag10) + Digital Stack + Snack Pack share ONE template `templates/product.single-mags.json` (loops `single-issues`, swaps client-side - do NOT make per-mag templates).
